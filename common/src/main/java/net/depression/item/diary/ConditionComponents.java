...
<<<<<<< HEAD
        if (player.getLevel().isRainingAt(player.blockPosition())) {
=======
        if (player.level().isRainingAt(player.blockPosition().atY(319))) {
...
>>>>>>> 79040de (0.1.4 Update)
    ...
    public static final ConditionComponent MODERATE_DEPRESSION_1_MOB_NEARBY = new ConditionComponent(player -> {
        if (player.getLevel().getEntities(player, player.getBoundingBox().inflate(10, 10, 10)).size() >= 3) {
