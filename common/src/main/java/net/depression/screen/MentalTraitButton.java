        ...
        if (screenY + height > 0 && screenY < screen.screenHeight) { // 只渲染在屏幕内的按钮
            ...
            if (screenY < 0) { //如果按钮被上面遮挡
                if (height + screenY > 1) {  //是否要画背景 ==2画的时候会崩溃
                    guiGraphics.blitNineSliced(WIDGETS_LOCATION, this.getX(), screen.baseY, this.getWidth(), height + screenY, 20, 4, 200, 20, 0, this.getTextureY());
                }
                ...
                if (screenY + disFrame + 24 > 0) { //是否要画框
                    ...
                    if (screenY + dis + 16 > 0) {  //是否要画图像
                        ...
                        renderTexture(guiGraphics, this.resourceLocation, this.getX() + dis, Math.max(this.getY() + dis, screen.baseY), this.xTexStart,
                ...
                if (screen.screenHeight - screenY > 1) { //是否要画背景 ==2画的时候会崩溃
                    guiGraphics.blitNineSliced(WIDGETS_LOCATION, this.getX(), screen.baseY + screenY, this.getWidth(), screen.screenHeight - screenY, 20, 4, 200, 20, 0, this.getTextureY());
                }
                ...
                if (screenY + disFrame < screen.screenHeight) { //是否要画框
                    ...
                    if (screenY + dis < screen.screenHeight) {  //是否要画图像
                        renderTexture(guiGraphics, this.resourceLocation, this.getX() + 8, this.getY() + 8, this.xTexStart,
                ...
                guiGraphics.blitNineSliced(WIDGETS_LOCATION, this.getX(), screen.baseY + screenY, this.getWidth(), height, 20, 4, 200, 20, 0, this.getTextureY());
                renderTexture(guiGraphics, FRAME_LOCATION, this.getX() + disFrame, this.getY() + disFrame, 0, 0, this.yDiffTex, 24, 24, 24, 24);
                renderTexture(guiGraphics, this.resourceLocation, this.getX() + dis, this.getY() + dis, this.xTexStart, this.yTexStart, this.yDiffTex, 16, 16, this.textureWidth, this.textureHeight);
                guiGraphics.drawCenteredString(Minecraft.getInstance().font, Component.translatable("depression.mental_trait." + id), this.getX() + charCenterOffset, this.getY() + (height / 2) - charHalfSize, 0xFFFFFF);
            }
        }
    }
