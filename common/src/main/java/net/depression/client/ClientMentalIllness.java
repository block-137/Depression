...
public class ClientMentalIllness {
    ...
    public void render(PoseStack poseStack, int x, int y) {
        ...
        if (elapsedTime > 10 && elapsedTime < 50) { //全闭眼
            ...
            GuiComponent.blit(poseStack, 0, 0, priority, 0, 0, x, y, 480, 360);
            return;
        }
        ...
        GuiComponent.blit(poseStack, 0, yLoc - 360, priority, 0, 0, x, 360, 480, 360); //k:显示优先级; f,g: （图片中的）起始偏移量; l,m: 实际显示大小; n,o: 图片大小
        RenderSystem.setShaderTexture(0, DROWSY_DOWN);
        GuiComponent.blit(poseStack, 0, y - yLoc, priority, 0, 0, x, 360, 480, 360);
        if (yLoc > 360) {
            RenderSystem.setShaderTexture(0, DROWSY_FULL);
            GuiComponent.blit(poseStack, 0, 0, priority, 0, 0, x, yLoc - 360, 480, 360);
            GuiComponent.blit(poseStack, 0, y - (yLoc - 360), priority, 0, 0, x, yLoc - 360, 480, 360);
        }
    }
}
